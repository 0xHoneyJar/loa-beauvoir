...
import { LABELS, getLabelsWithPrefix, hasLabel, hasLabelWithPrefix } from "./labels";
...
export class GapDetector {
  ...
  constructor(executor: IBrExecutor, config?: GapDetectionConfig) {
    ...
    this.staleHandoffMs = config?.staleHandoffThresholdMs ?? DEFAULT_STALE_HANDOFF_MS;
    this.orphanedTaskMs = config?.orphanedTaskThresholdMs ?? DEFAULT_ORPHANED_TASK_MS;
  ...
  private async detectOrphanedTasks(
    ...
    try {
      ...
      for (const task of inProgressTasks) {
        ...
        const sessionLabels = getLabelsWithPrefix(labels, LABELS.SESSION_PREFIX);
        ...
        if (sessionLabels.length === 0) {
          ...
          if (taskAge > this.orphanedTaskMs) {
            gaps.push({
              ...
              suggestedAction: "Reset task to ready state, or resume with a new session",
              autoResolvable: true,
            });
          }
        }
    ...
    try {
      ...
      for (const bead of withHandoffs) {
        ...
        if (beadAge > this.staleHandoffMs) {
          ...
          const sessionLabels = getLabelsWithPrefix(bead.labels || [], LABELS.SESSION_PREFIX);
          const handoffLabels = getLabelsWithPrefix(bead.labels || [], LABELS.HANDOFF_PREFIX);
          ...
          if (handoffLabels.length >= sessionLabels.length) {
            gaps.push({
              ...
              suggestedAction: "Claim handoff with a new session, or reset task state",
    ...
    try {
      ...
      for (const cb of circuitBreakers) {
        gaps.push({
          ...
          suggestedAction: "Investigate failure cause, then resolve circuit breaker to resume",
          autoResolvable: false,
        });
      }
      ...
      if (this.verbose) {
        console.error(`[gap-detection] Error detecting circuit breakers: ${e}`);
      }
      return { gaps: [], scanned: 0 };
    }
  }
    ...
    try {
      ...
      if (sessions.size > 0) {
        const latestSessionTime = Math.max(
          ...Array.from(sessions.values()).map((s) => new Date(s.latest).getTime()),
        );

        const unsessioned = allBeads.filter((b) => {
          const labels = b.labels || [];
          const hasSession = hasLabelWithPrefix(labels, LABELS.SESSION_PREFIX);
          const modifiedAfterLastSession = new Date(b.updated_at).getTime() > latestSessionTime;
          return !hasSession && modifiedAfterLastSession && b.status === "open";
        });
        ...
        if (unsessioned.length > 0) {
          gaps.push({
            ...
            suggestedAction: "Review out-of-band changes and attach to current session if relevant",
            autoResolvable: false,
          });
        }
      }
      ...
      if (this.verbose) {
        console.error(`[gap-detection] Error detecting session sequence gaps: ${e}`);
      }
      return { gaps: [], scanned: 0 };
    }
  }
      ...
      switch (gap.type) {
        case "orphaned_task": {
          for (const beadId of gap.affectedBeadIds) {
            ...
            await this.executor.exec(`label remove '${beadId}' '${LABELS.SPRINT_IN_PROGRESS}'`);
            await this.executor.exec(`label add '${beadId}' '${LABELS.STATUS_READY}'`);
          }
          return true;
        }
        default:
 ...
 */
export function createGapDetector(executor: IBrExecutor, config?: GapDetectionConfig): GapDetector {
