...
describe("getLabelsWithPrefix", () => {
  const testLabels = ["sprint:in_progress", "sprint:pending", "session:abc123", "epic"];
  ...
  it("should return single matching label", () => {
    expect(getLabelsWithPrefix(testLabels, "session:")).toEqual(["session:abc123"]);
  });
...
describe("deriveRunState", () => {
  ...
  it("should return HALTED even with other labels", () => {
    const labels = [LABELS.RUN_CURRENT, LABELS.SPRINT_IN_PROGRESS, LABELS.CIRCUIT_BREAKER];
    expect(deriveRunState(labels)).toBe("HALTED");
  });
...
describe("Integration", () => {
  ...
  it("should track session handoffs", () => {
    ...
    expect(getLabelsWithPrefix(allLabels, LABELS.HANDOFF_PREFIX)).toEqual(["handoff:session-abc"]);
  });
});
