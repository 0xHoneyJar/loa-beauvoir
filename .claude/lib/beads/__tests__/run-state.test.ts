...
import { BeadsRunStateManager, createBeadsRunStateManager } from "../run-state";
...
describe("BeadsRunStateManager", () => {
  ...
  describe("completeSprint", () => {
    it("should remove in_progress, add complete, and close bead", async () => {
      ...
      expect(mockExecutor.callHistory).toContainEqual(expect.stringContaining(`close 'sprint-1'`));
    });
  });
  ...
  describe("resumeRun", () => {
    it("should resolve all active circuit breakers", async () => {
      ...
      expect(mockExecutor.callHistory).toContainEqual(expect.stringContaining(`close 'cb-001'`));
      expect(mockExecutor.callHistory).toContainEqual(
  ...
  describe("getSameIssueCount (targeted query optimization)", () => {
    it("should use targeted query when issueHash provided", async () => {
      ...
      mockExecutor.mockResponse(`--label '${LABELS.CIRCUIT_BREAKER}' --label 'issue:abc123'`, {
        success: true,
        stdout: JSON.stringify([
          createMockBead({
            id: "cb-001",
    ...
    it("should fallback to full scan when targeted query returns empty", async () => {
      ...
      mockExecutor.mockResponse(`--label '${LABELS.CIRCUIT_BREAKER}' --label 'issue:xyz789'`, {
        success: true,
        stdout: "[]",
        stderr: "",
        exitCode: 0,
      ...
      mockExecutor.mockResponse(`--label '${LABELS.CIRCUIT_BREAKER}' --json`, {
        success: true,
        stdout: JSON.stringify([
          createMockBead({
            id: "cb-old",
    ...
    it("should prevent malicious issueHash from reaching shell (injection prevention)", async () => {
      ...
      for (const payload of injectionPayloads) {
        ...
        const targetedCalls = mockExecutor.callHistory.filter((c) => c.includes("issue:"));
