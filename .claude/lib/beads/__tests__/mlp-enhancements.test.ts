...
describe("Phase 2: Lineage Labels", () => {
  ...
  describe("createBranchedFromLabel", () => {
    it("should create branched-from label", () => {
      expect(createBranchedFromLabel("task-original")).toBe("branched-from:task-original");
    });
  });
  ...
  describe("parseLineageTarget", () => {
    ...
    it("should parse branched-from target", () => {
      expect(parseLineageTarget("branched-from:task-parent")).toBe("task-parent");
    });
  ...
  describe("getSupersedesTargets", () => {
    it("should extract all superseded bead IDs", () => {
      const labels = ["supersedes:task-v1", "supersedes:task-v2", "sprint:in_progress"];
  ...
  describe("getBranchedFromSources", () => {
    ...
    it("should handle multiple branch sources", () => {
      const labels = ["branched-from:task-a", "branched-from:task-b", "epic"];
  ...
  describe("lineage lifecycle integration", () => {
    it("should track task supersession chain", () => {
      ...
      const v2Labels = ["sprint:1", "sprint:in_progress", createSupersedesLabel("task-v1")];
    ...
    it("should track task split (branching)", () => {
      ...
      const subtask1Labels = ["sprint:1", createBranchedFromLabel("task-original")];
      const subtask2Labels = ["sprint:1", createBranchedFromLabel("task-original")];

      expect(getBranchedFromSources(subtask1Labels)).toEqual(["task-original"]);
      expect(getBranchedFromSources(subtask2Labels)).toEqual(["task-original"]);
    });
  });
});
...
describe("Phase 3: Classification & Confidence Labels", () => {
  ...
  describe("deriveClassification", () => {
    ...
    it("should prioritize by severity (blocker > decision > discovery > context > routine)", () => {
      expect(deriveClassification(["class:decision", "class:blocker"])).toBe("blocker");
      expect(deriveClassification(["class:discovery", "class:decision"])).toBe("decision");
      expect(deriveClassification(["class:context", "class:discovery"])).toBe("discovery");
      expect(deriveClassification(["class:routine", "class:context"])).toBe("context");
    });
  });
  ...
  describe("deriveConfidence", () => {
    ...
    it("should prioritize explicit > derived > stale", () => {
      expect(deriveConfidence(["confidence:explicit", "confidence:stale"])).toBe("explicit");
      expect(deriveConfidence(["confidence:derived", "confidence:stale"])).toBe("derived");
    });
  });
...
describe("Phase 1: Gap Detection", () => {
  ...
  describe("detect()", () => {
    ...
    it("should detect orphaned in-progress tasks", async () => {
      ...
      const orphanGaps = result.gaps.filter((g) => g.type === "orphaned_task");
    ...
    it("should detect unresolved circuit breakers", async () => {
      ...
      const cbGaps = result.gaps.filter((g) => g.type === "unresolved_circuit_breaker");
  ...
  describe("autoResolve()", () => {
    it("should resolve orphaned tasks by resetting labels", async () => {
      ...
      expect(mockExecutor.callHistory.some((c) => c.includes("label remove"))).toBe(true);
      expect(mockExecutor.callHistory.some((c) => c.includes("label add"))).toBe(true);
    });
...
describe("Phase 4: Context Compiler", () => {
  ...
  describe("compile()", () => {
    it("should include the target task with highest priority", async () => {
      ...
      const target = result.included.find((s) => s.bead.id === "target-task");
    ...
    it("should always include circuit breakers", async () => {
      ...
      mockExecutor.mockResponse("show", jsonResponse(targetTask));
      mockExecutor.mockResponse("circuit-breaker", jsonResponse([circuitBreaker]));
    ...
    it("should exclude stale beads by default", async () => {
      ...
      mockExecutor.mockResponse("show", jsonResponse(targetTask));
      mockExecutor.mockResponse("epic:sprint-1", jsonResponse([targetTask, staleBead]));

      const result = await compiler.compile("task-1");

      const staleExcluded = result.excluded.find((e) => e.bead.id === "stale-1");
    ...
    it("should exclude low-scoring routine beads", async () => {
      ...
      mockExecutor.mockResponse("show", jsonResponse(targetTask));
      mockExecutor.mockResponse("epic:sprint-1", jsonResponse([targetTask, routineBead]));

      const result = await compiler.compile("task-1");

      const routineExcluded = result.excluded.find((e) => e.bead.id === "routine-1");
    ...
    it("should prioritize decision beads over context beads", async () => {
      ...
      mockExecutor.mockResponse("class:decision", jsonResponse([decisionBead]));

      const result = await compiler.compile("task-1");

      const decisionIdx = result.included.findIndex((s) => s.bead.id === "decision-1");
      const contextIdx = result.included.findIndex((s) => s.bead.id === "context-1");
    ...
    it("should include compilation statistics", async () => {
      mockExecutor.mockResponse("show", jsonResponse(createMockBead({ id: "task-1" })));
    ...
    it("should boost recently updated beads", async () => {
      ...
      mockExecutor.mockResponse("show", jsonResponse(targetTask));
      mockExecutor.mockResponse("epic:sprint-1", jsonResponse([oldBead, recentBead, targetTask]));

      const result = await compiler.compile("task-1");

      const recentScored = result.included.find((s) => s.bead.id === "recent");
    ...
    it("should boost explicit confidence beads", async () => {
      ...
      const explicitScored = result.included.find((s) => s.bead.id === "explicit");
