...
export class ContextCompiler {
  ...
  async compile(taskBeadId: string): Promise<ContextCompilationResult> {
    ...
    for (const ex of excluded) {
      excludedByReason[ex.exclusionReason] = (excludedByReason[ex.exclusionReason] || 0) + 1;
    }
    ...
    return {
      ...
      stats: {
        ...
        utilization: this.tokenBudget > 0 ? usedTokens / this.tokenBudget : 0,
      },
  ...
  private async fetchRelevantBeads(taskBeadId: string): Promise<Bead[]> {
    ...
    if (task) {
      ...
      for (const epicLabel of epicLabels) {
        ...
        const sprintBeads = await this.queryBeads(`list --label '${epicLabel}' --json`);
        if (sprintBeads) {
  ...
  private scoreBeads(beads: Bead[], taskBeadId: string): ScoredBead[] {
    ...
    return beads.map((bead) => {
      ...
      if (!reason) {
        reason = classification ? `Classified as ${classification}` : "Unclassified bead";
      }
  ...
  private checkExclusion(scored: ScoredBead): ExclusionReason | null {
    ...
    if (!this.includeSuperseded) {
      ...
      if (scored.bead.status === "closed" && hasLabelWithPrefix(labels, LABELS.SUPERSEDES_PREFIX)) {
