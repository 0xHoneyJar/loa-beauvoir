 ...
 */
export const ALLOWED_TYPES = new Set(["task", "bug", "feature", "epic", "story", "debt", "spike"]);
...
export function validateType(type: unknown): asserts type is string {
  ...
  if (!ALLOWED_TYPES.has(type)) {
    throw new Error(`Invalid type: must be one of ${Array.from(ALLOWED_TYPES).join(", ")}`);
  }
}
 ...
 */
export function validatePriority(priority: unknown, min = 0, max = 10): asserts priority is number {
