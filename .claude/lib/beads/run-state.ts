...
export class BeadsRunStateManager implements IBeadsRunStateManager {
  ...
  async getRunState(): Promise<RunState> {
    try {
      ...
      const runs = await this.queryBeadsJson<Bead[]>(`list --label '${LABELS.RUN_CURRENT}' --json`);
  ...
  async getCurrentSprint(): Promise<SprintState | null> {
    try {
      ...
      const tasks = await this.queryBeadsJson<Bead[]>(`list --label 'epic:${sprint.id}' --json`);

      const completedTasks = (tasks || []).filter((t) => t.status === "closed").length;
      const currentTask = (tasks || []).find((t) => t.labels?.includes("in_progress"));
  ...
  async getSprintPlan(): Promise<SprintState[]> {
    try {
      ...
      for (const epic of epics) {
        ...
        // Count tasks
        const tasks = await this.queryBeadsJson<Bead[]>(`list --label 'epic:${epic.id}' --json`);
